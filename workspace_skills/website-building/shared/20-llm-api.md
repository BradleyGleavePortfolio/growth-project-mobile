...
## Media Generation — SDK Helper Scripts

Media generation (image, video, audio) uses a separate SDK (`pplx.python.sdks.llm_api`). **Do not use the Anthropic SDK for media.** Ready-to-use async helper scripts are in `shared/llm-api/`. **Read the relevant file, then copy it into your project directory** and import it from your FastAPI handlers.
...
Only `gemini_2_5_pro_tts` is supported — ElevenLabs is discontinued. Transcription is not currently available.

**Multi-speaker dialogue:** Use `generate_dialogue()` with a list of `{"speaker": "voice", "text": "..."}` dicts. Max 2 voices.

**Gemini voices:** achernar, achird, algenib, algieba, alnilam, aoede, autonoe, callirrhoe, charon, despina, enceladus, erinome, fenrir, gacrux, iapetus, kore, laomedeia, leda, orus, pulcherrima, puck, rasalgethi, sadachbia, sadaltager, schedar, sulafat, umbriel, vindemiatrix, zephyr, zubenelgenubi
