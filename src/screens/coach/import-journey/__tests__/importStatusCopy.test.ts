...
it('resolves every plain P2 string and pins its approved placeholders', () => {
  const visit = (value: unknown, prefix: string) => {
    if (typeof value === 'string') {
      const expected = /^(progress\.(receipts|native)|result\.unconfirmed)\.(zero|one|other)$/.test(prefix)
        ? ['count'] : prefix === 'progress.lastUpdate' ? ['time'] : prefix === 'result.scope' ? ['scopeLabel'] : [];
      expect(Array.from(value.matchAll(/\{([^{}]+)\}/g), match => match[1])).toEqual(expected);
      if (expected.length === 0) expect(t(prefix as Extract<ImportJourneyCopyKey, `progress.${string}` | `result.${string}`>)).toBe(value);
...
it.each(['receipts', 'nativeClients', 'unconfirmedClients'] as const)('formats explicit zero/one/other %s quantities, never unknown as zero', kind => {
  for (const count of [0, 1, 12000, Number.MAX_SAFE_INTEGER]) {
    const formatted = importQuantityCopy(kind, count, 'en-US');
    expect(formatted).toContain(`\u2068${new Intl.NumberFormat('en-US').format(count)}\u2069`);
...
it('uses only supplied observation time and reviewed scope, never render time or arbitrary prose', () => {
  ...
  expect(importObservationCopy(observed)).not.toMatch(/[{}]/);
  ...
  expect(importCheckedScopeCopy('selectedClientRecords')).not.toMatch(/[{}]/);
