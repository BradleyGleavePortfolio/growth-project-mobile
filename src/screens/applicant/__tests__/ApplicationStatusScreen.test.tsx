...
describe('ApplicationStatusScreen', () => {
  ...
  it('uses theme tokens and not hardcoded colour hex values', () => {
    ...
    // Allow `#` only inside comments.
    const nonCommentLines = SCREEN_SRC
      .split('\n')
      .filter((l) => !l.trim().startsWith('//') && !l.trim().startsWith('*'));
    const hexInCode = nonCommentLines.some((l) => /#[0-9a-fA-F]{3,6}/.test(l));
    expect(hexInCode).toBe(false);
  });

  it('has STATUS_LABEL for every status value', () => {
    for (const status of ALL_STATUSES) {
      expect(SCREEN_SRC).toContain(`'${status}':`);
    }
  });

  it('has STATUS_DESCRIPTION for every status value', () => {
    // Both STATUS_LABEL and STATUS_DESCRIPTION use the same key format.
    // Simply verify the string appears at least twice (once per map).
    for (const status of ALL_STATUSES) {
      const occurrences = (SCREEN_SRC.match(new RegExp(`'${status}':`, 'g')) ?? []).length;
...
describe('talentMarketplaceApi', () => {
  ...
  it('does not contain any API keys or hardcoded URLs', () => {
