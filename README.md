...
4. **Profile** — accessibility label `Profile and more`, route name `MoreTab`. Wraps `MoreStack`, which houses `MoreIndex`, `ProfileMain`, `Recipes`, `RecipeDetail`, `GroceryList`, `ShoppingList`, `PrepGuide`, `Fast`, `Community`, `Progress`, `Settings`, `Widgets`, `Report`, `Learn`, `Plan`, `TrustCenter`, and `Preferences`. There is no global floating chat widget — the AI surface is `AIGuideScreen` under the same stack. The `TrophyShare` route was removed in #63 alongside the celebration chrome.
