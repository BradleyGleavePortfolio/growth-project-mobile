...
Rules referenced use the post-PR numbering (Rules 1–29).
...
| 4 | XSS via Unescaped Output | P0 | R5, R17 (scrub outbound error strings / output sanitisation) | PARTIAL |
| 5 | Broken Object-Level Authorization (IDOR) | P0 | R22 (RBAC server-authoritative), R15 (user-scoped keys) | COVERED |
| 6 | Missing Rate Limiting on Auth Endpoints | P0 | R5 (avoid 50 failures) | PARTIAL |
...
| 11 | Missing / Wildcard CORS Configuration | P1 | R5 (avoid 50 failures) | PARTIAL |
...
| 21 | N+1 Query Problem | P0 | R5, R1 (decacorn quality), R7 (enterprise grade) | PARTIAL |
| 22 | Missing DB Indexes on Queried Columns | P0 | R5, R1, R7 | PARTIAL |
...
|---|---------|-----|----------------|
| 4 | XSS via Unescaped Output | No explicit output-sanitisation rule | Add Rule 30: All user-supplied strings rendered to UI must pass through an approved sanitisation function (DOMPurify / equivalent); dangerouslySetInnerHTML is banned. |
| 6 | Missing Rate Limiting on Auth Endpoints | No rate-limit mandate | Add Rule 31: Every auth and paid-API endpoint must have rate-limiting middleware configured before merge; limits are documented in the PR. |
...
| 11 | Missing / Wildcard CORS | No explicit CORS rule | Add Rule 32: CORS must use an explicit origin allowlist; wildcard origin (`*`) is banned on any endpoint that handles auth tokens or user data. |
...
| 21 | N+1 Query Problem | No query-performance rule | Add Rule 33: No database query may appear inside a loop; use eager loading, batch fetching, or DataLoader. |
| 22 | Missing DB Indexes | No index rule | Add to Rule 33's explanation: Every foreign key and every column used in a WHERE/ORDER BY on high-volume tables must have an index. |
...
| 2 | **R1 / R7** — Decacorn quality / 99.99% uptime | 21, 22, 23, 24, 25, 27, 35, 47 | 8 |
| 3 | **R14** — Always latest plumbing / CI | 1, 10, 18, 20, 26, 37, 41, 48, 49 | 9 |
| 4 | **R22** — RBAC server-authoritative | 2, 5, 7, 9 | 4 |
| 5 | **R19** — Every mutation idempotent | 28, 29, 44 | 3 |
| 6 | **R17** — Scrub outbound error strings | 12, 36 | 2 |
| 7 | **R18** — No fabricated success | 30, 36 | 2 |
| 8 | **R20** — One source of truth | 28, 30, 31, 44 | 4 |
| 9 | **R24** — Dead code dies same PR | 16, 24 (cleanup), 40, 43 | 4 |
| 10 | **R29** — Backend contract drift fails build | 8, 18, 19, 46 | 4 |
| 11 | **R25** — Audit every caller before contract change | 8, 19, 23 | 3 |
| 12 | **R26** — Tests prove behaviour, not strings | 17 | 1 |
| 13 | **R9** — No raw errors to users | 12, 33, 36, 50 | 4 |
| 14 | **R6** — Never kick the can | 16, 40 | 2 |
| 15 | **R23** — Hooks unconditional | 31, 32 | 2 |
...
All 50 failures are addressed at minimum by the meta-rule R5. The 28 PARTIAL failures are ones where R5 is the only direct guardrail and no specific standing rule mandates the concrete practice. The recommendations above in "Uncovered failures" propose Rules 30–33 to close the most important of these gaps.
